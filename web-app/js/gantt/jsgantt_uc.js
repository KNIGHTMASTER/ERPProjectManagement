...
JSGantt.TaskItem = function(pID, pName, pStart, pEnd, pColor, pLink, pMile, pRes, pComp, pPrice, pGroup, pParent, pOpen, pDepend, pCaption)
{
var vID    = pID;  
var vName  = pName;   
var vStart = new Date();	  
...
var vVisible  = 1;
    ...
    this.getPriceVal  = function(){ if(vPrice) return vPrice; else return 0; };
    this.getCompStr  = function(){ if(vComp) return vComp+'%'; else return ''; };
    this.getPriceStr  = function(){ if(vPrice) return vPrice; else return ''; };
   ...
   this.setCompVal  = function(pCompVal){ vComp = pCompVal;};
	this.setPriceVal  = function(pPriceVal){ vPrice = pPriceVal;};
...
if(vFormatArr.length>4){vFormatArr.length=4;}
 ...
 this.setShowPrice = function(pShow) { vShowPrice = pShow; };
...
this.getShowPrice = function(){ return vShowPrice };
 ...
 this.Draw = function()
   {
	 ...
	 if(vShowPrice!=1) vNameWidth+=vStatusWidth;
		   if(vShowStartDate!=1) vNameWidth+=vStatusWidth;
		   if(vShowEndDate!=1) vNameWidth+=vStatusWidth;
        
		   // DRAW the Left-side of the chart (names, resources, comp%)
         vLeftTable =
            '<DIV class=scroll2 id=leftside style="width:' + vLeftWidth + 'px"><TABLE cellSpacing=0 cellPadding=0 border=0><TBODY>' +
	 ...
	 if(vShowPrice==1) vLeftTable += '  <TD style="WIDTH: ' + vStatusWidth + 'px; HEIGHT: 17px"></TD>' ;
         ...
         vLeftTable +=
            ...
            '  <TD style="BORDER-TOP: #efefef 1px solid; WIDTH: ' + vNameWidth + 'px; HEIGHT: 20px"><NOBR></NOBR>Project management</TD>' ;
         ...
         if(vShowPrice==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>Rp.Price</TD>' ;       
	 if(vShowStartDate==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>Start Date</TD>' ;
         ...
         vLeftTable += '</TR>';
            ...
            for(i = 0; i < vTaskList.length; i++)
            {
               ...
               if(vShowRes ==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: right; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 11px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + vTaskList[i].getResource() + '</NOBR></TD>' ;
               if(vShowDur ==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: right; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 11px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + vTaskList[i].getDuration(vFormat) + '</NOBR></TD>' ;
               if(vShowComp==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: right; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 11px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + vTaskList[i].getCompStr()  + '</NOBR></TD>' ;
               if(vShowPrice==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: right; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 11px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + vTaskList[i].getPriceStr()  + '</NOBR></TD>' ;
                         
if(vShowStartDate==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: right; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 11px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + JSGantt.formatDateStr( vTaskList[i].getStart(), vDateDisplayFormat) + '</NOBR></TD>' ;
               if(vShowEndDate==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: right; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 11px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + JSGantt.formatDateStr( vTaskList[i].getEnd(), vDateDisplayFormat) + '</NOBR></TD>' ;
            ...
            vLeftTable += '</TD></TR>' +
              '<TR><TD border=1 colspan=5 align=left style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 11px; BORDER-LEFT: #efefef 1px solid; height=18px">&nbsp;&nbsp;Powered by <a href=http://www.bangOne.com>JSGantt-support by: bangOne</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
            vLeftTable +='<br>&nbsp;&nbsp;Format:&nbsp;&nbsp;';
            if (vFormatArr.join().indexOf("minute")!=-1) {
            if (vFormat=='minute') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="minute">Minute';
            ...
            if (vFormat=='hour') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="hour" checked style="width:10px;margin=0px;">Hour';
            else                vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("hour",'+vGanttVar+'); VALUE="hour" style="width:10px;margin=0px;">Hour';
			}
			
			if (vFormatArr.join().indexOf("day")!=-1) { 
            if (vFormat=='day') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="day" checked style="width:10px;margin=0px;" >Day';
            else                vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("day",'+vGanttVar+'); VALUE="day" style="width:10px;margin=0px;">Day';
			}
			
			if (vFormatArr.join().indexOf("week")!=-1) { 
            if (vFormat=='week') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="week" checked style="width:10px;margin=0px;" >Week';
            else                vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("week",'+vGanttVar+') VALUE="week" style="width:10px;margin=0px;">Week';
			}
			
			if (vFormatArr.join().indexOf("month")!=-1) { 
            if (vFormat=='month') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="month" checked style="width:10px;margin=0px;" >Month';
            else                vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("month",'+vGanttVar+') VALUE="month" style="width:10px;margin=0px;">Month';
			}
			
			if (vFormatArr.join().indexOf("quarter")!=-1) { 
            if (vFormat=='quarter') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="quarter" checked style="width:10px;margin=0px;">Quarter';
            else                vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("quarter",'+vGanttVar+') VALUE="quarter" style="width:10px;margin=0px;">Quarter';
			}
...
//            vLeftTable += '<INPUT TYPE=RADIO NAME="other" VALUE="other" style="display:none"> .';
            ...
            // Draw the Chart Rows
            vRightTable = 
            '<TD style="width:auto;" vAlign=top bgColor=#ffffff>' +
	       ...
	       else if (vFormat=='minute')
	       {
            ...
            if( vTaskList[i].getMile()) {
               ...
               if(vTaskList[i].getCompVal() < 100)
			      ...
			      case 'Price':      vCaptionStr = vTaskList[i].getPriceStr();  break;
		                     }
                           //vRightTable += '<div style="FONT-SIZE:12px; position:absolute; left: 6px; top:1px;">' + vCaptionStr + '</div>';
                           vRightTable += '<div class="res" style="FONT-SIZE:8px; position:absolute; top:2px; width:120px; left:12px">' + vCaptionStr + '</div>';
	                  };
               ...
               if( vTaskList[i].getGroup()) {
			      ...
			      case 'Price':   vCaptionStr = vTaskList[i].getPriceStr();  break;
                           ...
                           //vRightTable += '<div style="FONT-SIZE:12px; position:absolute; left: 6px; top:1px;">' + vCaptionStr + '</div>';
                           vRightTable += '<div class="res" style="FONT-SIZE:8px; position:absolute; top:-3px; width:180px; left:' + (Math.ceil((vTaskRight) * (vDayWidth) - 1) + 6) + 'px">' + vCaptionStr + '</div>';
	                  };
...
JSGantt.processRows = function(pList, pID, pRow, pLevel, pOpen)
{
   ...
   if(pRow >= 0) {
      ...
      pList[pRow].setCompVal(Math.ceil(vCompSum/vNumKid)); //pelajari pembagian prosentase
      pList[pRow].setPriceVal(Math.ceil(vPriceSum));	
   }
...
JSGantt.AddXMLTask = function(pGanttVar){
	...
	for(var i=0;i<n;i++) {
		...
		if(pID!=0){
			...
			try { pPrice = Task[i].getElementsByTagName("pPrice")[0].childNodes[0].nodeValue;
			} catch (error) {pPrice =0;}
			pPrice *= 1;
			...
			pGanttVar.AddTaskItem(new JSGantt.TaskItem(pID , pName, pStart, pEnd, pColor,  pLink, pMile, pRes,  pComp, pPrice, pGroup, pParent, pOpen, pDepend,pCaption));
		}
	}
};
...
// Manually parse the file as it is loads quicker
	if (XMLLoader.readyState == 4) {
		...
		for(var i=1;i<n;i++) {
			...
			var te = Task.split(/<pPrice>/i);
			if(te.length> 2){var pPrice=te[1];} else {var pPrice = 0;}	
			pPrice  *= 1;

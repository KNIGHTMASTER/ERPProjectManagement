	...
	<body>
		...
		<div id="show-contract" class="content scaffold-show" role="main">
			...
			<ol class="property-list contract">
				...
				<g:if test="${contractInstance?.projectName}">
				<li class="fieldcontain">
					<span id="projectName-label" class="property-label"><g:message code="contract.projectName.label" default="Project Name" /></span>
					
						<span class="property-value" aria-labelledby="projectName-label"><g:link controller="projectCharter" action="show" id="${contractInstance?.projectName?.id}">${contractInstance?.projectName?.encodeAsHTML()}</g:link></span>
